...
- [[#plugins-and-plugin-settings][Plugins and plugin settings]]
...
I decided to use a plugin manager after all, since I started using more than one plugins. I first decided to use [[https://github.com/zplug/zplug][zplug]], since it seemed to me as the simplest to use overall, but it was slower than even Oh-my-zsh (one time I counted, and it took half a minute with only 3 plugins!). I then decided to use [[https://github.com/zdharma/zplugin][zplugin]] instead, since it was faster, even though I'm not going to use the vast majority of its features.\\
...
#+end_src
* Aliases
Mainly aliases for variants of ls and grep, but also one to have quick access to my own preferences on tty-clock.
#+begin_src sh
...
#+end_src
Also alias the dollar sign so that it is ignored if it is in the beginning of a command, something that would happen most likely by mistake (idea from [[https://github.com/zpm-zsh/undollar][here]]).
#+begin_src sh
alias \$=''
#+end_src
