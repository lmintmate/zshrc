...
- [[#zsh-plugins-with-zplugin][Zsh plugins with zplugin]]
  ...
  - [[#plugins][Plugins]]
    - [[#crasis-semigraphical-zplugin-manager][Crasis (semigraphical zplugin manager)]]
...
*** Crasis (semigraphical zplugin manager)
First add [[https://github.com/zdharma/zplugin-crasis][zplugin-crasis]], which is an ncurses-style manager for zplugin plugins. With it one can easily add, enable/disable or remove plugins without having to edit the .zshrc file directly.
#+begin_src sh
...
#+end_src
In regards to ~zsh-auto-notify~, don't show notifications for the zplugin manager crasis, the text editor micro and the music player mocp, since it's normal that these run for more than 10 seconds.
#+begin_src sh
