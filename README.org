...
- [[#plugins-and-plugin-settings][Plugins and plugin settings]]
  - [[#bootstrap-the-plugin-manager-zplug][Bootstrap the plugin manager, zplug]]
...
** Bootstrap the plugin manager, zplug
I decided to use a plugin manager after all, since I started using more than one plugins. I decided to use [[https://github.com/zplug/zplug][zplug]], since it seems to me as the simplest to use overall.\\
First download and source zplug itself.
#+begin_src sh
if [[ ! -d ~/.zplug ]];then
